...
export function DescendantProvider({ items, ...props }) {
  ...
  // The exhaustive-deps warning here is because we pass forceUpdate with no
  // dependencies, which *could* lead to infinite updates if we weren't checking
  // the ref value first, so that's why we are ignoring tiny Dan in this case.
  // (If this is confusing, we like to think of the eslint hooks rules as a tiny
  // Dan Abramov who sits on your shoulder and warns you when you forget to
  // pass the correct dependencies into your hooks!)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useLayoutEffect(() => {
    if (assigning.current) {
      // At this point all of the children have pushed into the array so we set
      // assigning to false and force an update. Since we're in useLayoutEffect,
      // we won't get a flash of rendered content, it will all happen
      // synchronously. And now that this is false, children won't push into the
      // array on the forceUpdate
