...
| [`as`](#menulink-as)                     | `string | Component` | true     |
...
- `MenuButton`: Activates `Menu` in `onMouseDown` or `onKeyDown` (<kbd>Enter</kbd> or <kbd>Space</kbd> keys)
- `MenuItem`: Selects itself in `onMouseUp` or `onKeyDown` (<kbd>Enter</kbd> or <kbd>Space</kbd> keys)
- `MenuLink`: Selects itself in `onMouseUp` or `onKeyDown` (<kbd>Enter</kbd> or <kbd>Space</kbd> keys).
...
| <kbd>Space</kbd>     | Open/close while focused on the `MenuButton`; Selects an item when navigating. |
