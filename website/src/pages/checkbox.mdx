...
- Source: https://github.com/reach/reach-ui/tree/master/packages/checkbox
- WAI-ARIA: https://www.w3.org/TR/wai-aria-practices-1.1/#checkbox
...
If you don't need an indeterminate state, you should probably just use a native HTML input for your checkboxes. But of course, sometimes designers have some other ideas that call for a custom solution. In that case, the `CustomCheckbox` component provides a customizable wrapper element that can be styled to fit your needs.
...
> Accessibility Note: If you use our `CustomCheckbox` component, you will still need to ensure that your styles follow the guidelines outlined in the [WAI-ARIA specifications for checkboxes](https://www.w3.org/TR/wai-aria-practices-1.1/#checkbox). Pay special attention to focus styles for keyboard navigation. Our default styles provide focus styles by default.
...
// jsx-demo
function Example() {
  let [favoriteCondiments, setFavoriteCondiments] = useState({
    mayo: true,
    mustard: true,
    ketchup: false
  });
  ...
  function handleAllCondimentsChange(event) {
    setToppings(
  ...
  return (
    <fieldset>
      <label>
        <MixedCheckbox
          value="condiments"
          checked={allCondimentsChecked}
          onChange={handleAllCondimentsChange}
        />
...
`MixedCheckbox` inherits its props from the `React.InputHTMLAttributes<HTMLInputElement>` interface, with additional context documented below.
...
_Type_: `boolean` | `"mixed"`

Whether or not the checkbox is checked or in a `mixed` (indeterminate) state.

##### MixedCheckbox onChange

_Type_: `func: (event: React.ChangeEvent<HTMLInputElement>) => void`

The callback that is fired when the checkbox value is changed.

### useMixedCheckbox

_Type_: `func: (ref: React.RefObject<HTMLInputElement>, args) => [inputProps, stateData]`

A hook that can be used to turn any HTML input component into a tri-state checkbox. You must create a ref and pass it along with additional arguments to return a props object and state-related data.
...
function Example({ disabled = false }) {
  ...
  return (
    <div>
      ...
      <hr />
      Current state is: <pre>{stateData.checked}</pre>
    </div>
  );
}
```

#### useMixedCheckbox args
...
A `CustomCheckbox` inherits its props from the `React.InputHTMLAttributes<HTMLInputElement>` interface, with additional context documented below.
...
| [disabled](#customcheckboxcontainer-disabled)             | `boolean`              | false    | `false` |
| [onChange](#customcheckboxcontainer-onchange)             | `func`                 | false    |         |

##### CustomCheckboxContainer checked

Whether or not the checkbox is checked or in a `mixed` (indeterminate) state. This prop is assigend to the `CustomCheckboxContainer` and passed to the `CustomCheckboxInput` via the [React Context API](https://reactjs.org/docs/context.html).

##### CustomCheckboxContainer children

_Type_: `node` | `func: ({ checked: boolean | "mixed", inputRef: React.RefObject, focused: boolean }) => JSX.Element`
...
In uncontrolled components, `defaultChecked` represents the initial checked state for a `CustomCheckbox`. This prop is assigend to the `CustomCheckboxContainer` and passed to the `CustomCheckboxInput` via the [React Context API](https://reactjs.org/docs/context.html).
...
Whether or not the custom checkbox's input element is disabled. This prop is assigend to the `CustomCheckboxContainer` and passed to the `CustomCheckboxInput` via the [React Context API](https://reactjs.org/docs/context.html).

##### CustomCheckboxContainer onChange

_Type_: `func: (event: React.ChangeEvent<HTMLInputElement>) => void`

The callback that is fired when the checkbox value is changed. This prop is assigend to the `CustomCheckboxContainer` and passed to the `CustomCheckboxInput` via the [React Context API](https://reactjs.org/docs/context.html).
...
`MixedCheckbox` inherits its props from the `React.InputHTMLAttributes<HTMLInputElement>` interface, excluding:
