...
An accessible dropdown menu for the common "actions dropdown" design pattern.
...
```jsx
<MenuButton>
  Actions <span aria-hidden="true"><Gear /></span>
</MenuButton>
```
...
All props are spread to the underlying  element. Here we apply a `className` and `style` to the element.
...
<Menu>
  ...
  <MenuList
    className="gray-bg"
    style={{ boxShadow: "2px 2px 10px hsla(0, 0%, 0%, 0.25)" }}
  >
...
In this example the title attribute is passed down the element.
...
<Menu>
  ...
  <MenuList>
    <MenuItem
      title="Start a video with your friends"
      onSelect={() => {}}
    >Start Video</MenuItem>
    <MenuItem onSelect={() => {}}>Start Screenshare</MenuItem>
    <MenuItem onSelect={() => {}}>Send a Message</MenuItem>
  </MenuList>
</Menu>
...
<Menu>
  <MenuButton>
    Your Cats <span aria-hidden="true">▾</span>
  </MenuButton>
...
Handles linking to a different page in the menu. By default it works with Reach Router, but also accepts any other kind of Link.
...
Additionally, if other routers' `Link` component uses the `React.forwardRef` API, you can pass them in as well.
