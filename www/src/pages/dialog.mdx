...
| [accessibilityLabel](#dialog-accessibilitylabel) | string |
...
A hidden label for assistive devices.

If the label text is visible on screen inside the dialog, you should wrap the label inside a component with a unique `id` prop and provide a matching value to `aria-labelledby` on the Dialog instead of `accessibilityLabel`. Use `accessibilityLabel` when the name of the Dialog cannot be determined programmatically from its visible content, as there are cases where providing a visible label is not the desired user experience.
...
function Example(props) {
  return (
    <Dialog accessibilityLabel="Warning about next steps" isOpen={true}>
...
| [accessibilityLabel](#dialogcontent-accessibilitylabel) | string |
...
### DialogContent accessibilityLabel

A hidden label for assistive devices.

See [Dialog accessibilityLabel](#dialog-accessibilitylabel) for details.
