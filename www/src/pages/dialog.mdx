...
| [removeScroll](#dialog-removescroll)   | bool   |
...
<Component initialState={{ showDialog: false }}>
  {({ state, setState }) => (
    <div>
      ...
      <Dialog
        style={{ color: "red" }} isOpen={state.showDialog}
        onDismiss={() => setState({ showDialog: false })}
      >
...
The page scroll is disabled by default. This behavior can be controlled with the `removeScroll` prop.

```.jsx
<Component initialState={{ showDialog: false, hideScroll: true }}>
  {({ state, setState }) => (
    <div>
      <button onClick={() => setState({ showDialog: true, hideScroll: true })}>
        Open Dialog without scroll
      </button>
      <button onClick={() => setState({ showDialog: true, hideScroll: false })}>
      ...
      >
        ...
        <p>{state.hideScroll ? "No scroll" : "Scroll way!"}</p>
      </Dialog>
    </div>
  )}
</Component>
...
| [removeScroll](#dialogoverlay-removescroll)       | bool   |
...
Same as [Dialog `removeScroll`](#dialog-removescroll)
