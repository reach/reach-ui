...
A combo box is the combination of an `<input type="text"/>` and a list. The list is designed to help the user arrive at a value, but the value does not necessarily have to come from that list. Think of it more like an `<input type="text"/>` with some suggestions than a `<select/>`.
...
Sometimes your items need to be more than just text, in these cases you can pass children to `ComboboxOption`, and then render a `<ComboboxOptionText/>` to continue to get the built-in text highlighting to still work. Only the `value` is used to match, not the children.
    ...
    return (
      <div>
        ...
        <Combobox>
          ...
          />
          {results && (
            <ComboboxPopup className="shadow-popup">
...
If you pass `portal={false}` the popup will no longer be rendered in a portal, but in the same order as the React tree.
