...
This will save and read the index from local storage so that if you refresh the page you will be on the same tab as before.
...
(() => {
  function LocalStorageDemo() {
    ...
    if (tabIndex == null) {
      ...
      setTabIndex(parseInt(storedValue, 10));
    }

    const setValueWithStorage = index => {
      localStorage.setItem("tabIndex", String(index));
      setTabIndex(index);
    };

    return (
      <Tabs index={tabIndex} onChange={setValueWithStorage}>
        <TabList>
          <Tab>One</Tab>
          <Tab>Two</Tab>
          <Tab>Three</Tab>
        ...
        <TabPanels>
          <TabPanel>Click the tabs, then refresh the page.</TabPanel>
          <TabPanel>It should remember it.</TabPanel>
          <TabPanel>Sorry about the scrolling, we should fix that.</TabPanel>
        </TabPanels>
      </Tabs>
    );
  }
