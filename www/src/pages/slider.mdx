...
Slider expects `<SliderTrack>` as its sole child; The track will accept all additional slider sub-components as children.
...
`SliderTrack` expects `<SliderHandle>` (or a `<SliderHandle>` nested inside a `<SliderTooltip>`), at minimum, for the Slider to function. All other Slider subcomponents should be passed as children inside the `SliderTrack`.
...
`SliderTooltip` expects `<SliderHandle>` as its sole child.

### SliderTooltip getLabel

_Type_: `func: (value: number) => string`

A function used to set a human-readable text label based on the slider's current value. If the tooltip's text should be static, use the `label` prop instead.

### SliderTooltip label

_Type_: `string`

A static label to use inside the tooltip. If neither a `label` or `getLabel` prop is set, the Tooltip will default to the slider's `aria-valuetext` value if it is set, otherwise the slider's numeric value.
